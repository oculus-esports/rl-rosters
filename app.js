...
// UPDATE THIS WHEN A NEW ROCKET LEAGUE SEASON STARTS
...
async function saveTeam(directData = null) {
    ...
    if (directData) {
        ...
        // Auto-generate a beautiful fallback logo using their team initials if no file is uploaded
...
function renderAll() {
    ...
    teams.forEach(team => {
        ...
        let playersHTML = teamPlayers.map(p => {
            ...
            // Logic to default to handle if no alias is provided
            ...
            <div class="player-item" style="background: #1e1e24; border: 1px solid #2d2d33; border-radius: 8px; padding: 16px; margin-bottom: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.2);">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 10px; margin-bottom: 15px;">
                    ...
                    <div style="background: #25252b; padding: 12px; border-radius: 6px; text-align: left; border: 1px solid #333;">
                        ...
                        <div id="view-1v1-${p.id}">
                            <div style="font-size: 1.2em; font-weight: bold; color: #fff; display: flex; align-items: baseline; gap: 8px;">
                                <span>${p.duel_1v1_current_mmr || 0}</span>
                            </div>
                    ...
                    <div style="background: #25252b; padding: 12px; border-radius: 6px; text-align: left; border: 1px solid #333;">
                        ...
                        <div id="view-2v2-${p.id}">
                            <div style="font-size: 1.2em; font-weight: bold; color: #fff; display: flex; align-items: baseline; gap: 8px;">
                                <span>${p.doubles_2v2_current_mmr || 0}</span>
                            </div>
                    ...
                    <div style="background: #25252b; padding: 12px; border-radius: 6px; text-align: left; border: 1px solid #333; position: relative;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #2d2d33; padding-top: 12px;">
                    ...
                    <div class="${isAdmin ? '' : 'hidden'} admin-only" style="display: flex; gap: 8px; align-items: center;">
                        ${trackerLinkHTML}
                        <button id="edit-player-btn-${p.id}" onclick="enableInlinePlayerEdit('${p.id}')" style="background-color: #ff9800; color: black; padding: 4px 10px; font-size: 0.85em; font-weight: 600; border: none; border-radius: 4px; cursor: pointer;">Edit MMR</button>
        ...
        card.innerHTML = `
            <div class="team-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; border-bottom: 1px solid #333; padding-bottom: 15px; margin-bottom: 15px;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <div class="team-info">
                        <h3 style="margin: 0; font-size: 1.4em; color: #fff;" id="display-name-${team.id}">${team.name}</h3>
